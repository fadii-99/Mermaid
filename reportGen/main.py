...
def form():
    if request.method == 'POST':
        session['child_dob'] = request.form.get('child_dob')
        session['requester'] = request.form.get('requester')
        session['pronoun'] = request.form.get('pronoun')
        session['assesment_administered'] = request.form.get('assesment_administered')
        session['assesment_time'] = request.form.get('assesment_time')
        session['meet_teacher'] = request.form.get('meet_teacher')
        session['meet_parent'] = request.form.get('meet_parent')
        session['meet_therapy_team'] = request.form.get('meet_therapy_team')
        session['lessons_observed'] = request.form.get('lessons_observed')
        session['sensory_profile'] = request.form.get('sensory_profile')
        session['parent_assesment'] = request.form.get('parent_assesment')

        if session['parent_assesment'] == "Yes":
            selected_assessments = request.form.getlist('assessments[]')
            assessment_mapping = {
            ...
            session['selected_assessments'] = [assessment_mapping[assess] for assess in selected_assessments]

        return redirect(url_for('background'))

    return render_template('form.html', step='introduction')


@app.route('/background', methods=['GET', 'POST'])
def background():
    if request.method == 'POST':
        external_report_files = request.files.getlist('external_report_pdf')
        internal_referral_files = request.files.getlist('initial_referral_pdf')
        ...
        session['external_report_result'] = external_report_result
        session['initial_referral_result'] = initial_referral_result

        session['external_report_content'] = all_external_content  
        session['initial_referral_content'] = all_internal_content

        return redirect(url_for('views'))

    return render_template('form.html', step='background')


@app.route('/views', methods=['GET', 'POST'])
def views():
    if request.method == 'POST':
        parent_input = request.form.get('parent_input')
        teacher_input = request.form.get('teacher_input')
        ...
        session['views_result'] = views_result
        return redirect(url_for('assessment'))
    return render_template('form.html', step='views')
...
def assessment():
    if 'selected_assessments' not in session or not session['selected_assessments']:
        flash("No assessments selected. Please complete the introduction step.")
    ...
    if request.method == 'POST':
        ...
        selected_fields = []
        for idx, assessment in enumerate(session['selected_assessments']):
            if assessment['type'] == 'manual':
                fields = manual_assessment_fields.get(assessment['name'], [])
                selected_fields.extend(fields)

                for field in fields:
                    field_name = field['name']
                    manual_data[field_name] = request.form.get(field_name, "N/A")

            elif assessment['type'] == 'pdf':
                pdf_field = f"pdf_{idx + 1}"
                if pdf_field in request.files:
                    file = request.files[pdf_field]
                    if file and allowed_file(file.filename):
                        ...
                        pdf_texts[assessment['name']] = pdf_text
                        ...
                        pdf_results[assessment['name']] = extracted_tables

                        # pdf_texts = extract_text_from_pdf(file_path)
                        # pdf_texts[assessment['name']] = pdf_texts

        session['pdf_results'] = pdf_results  
        session['cleaned_tables'] = enhanced_tables 
        session['pdf_texts'] = pdf_texts
        ...
        print(f"\n\nAssessment Input: {assessment_input}")

        dynamic_template = generate_dynamic_prompt(selected_fields)
        print(f"\n\nDynamic Template: {dynamic_template}")
        ...
        assessment_chain = LLMChain(llm=llm, prompt=assessment_prompt)
        assessment_result = assessment_chain.run(assessment_input)
    ...
    return render_template(
        'form.html',
        step='assessment',
        manual_assessment_fields=manual_assessment_fields
    )

@app.route('/clinical_analysis', methods=['GET', 'POST'])
def clinical_analysis():

    if request.method == 'POST':
        selected_statements = []

        for checkbox, statement in checkbox_statements.items():
            if request.form.get(checkbox): 
                selected_statements.append(statement)

        print(f'\n\nSelected Assessments are : {selected_statements}')

        clinical_analysis_content = " ".join(selected_statements)
        print(f'\n\nClinical Analysis Input : {clinical_analysis_content}')
        
        summary_result = clinical_analysis_chain.run({"clinical_analysis_content": clinical_analysis_content})
        session['clinical_analysis_result'] = summary_result
        return redirect(url_for('report'))
    
    return render_template('form.html', step='clinical_analysis', checkbox_statements=checkbox_statements)

@app.route('/recommendations', methods=['GET', 'POST'])
def recommendations():
    intro_content = session.get('intro_content', '')
    external_report_content = session.get('external_report_result', '')
    initial_referral_content = session.get('initial_referral_result', '')
    views_content = session.get('views_result', '')
    assessment_content = session.get('assessment_result', '')
    clinical_analysis_content = session.get('clinical_analysis_result', '')
    summary_of_strengths_content = session.get('summary_of_strengths_result', '')
    summary_of_needs_content = session.get('summary_of_needs_result', '')
    ...
    session['recommendations_result'] = recommendations_result

    return redirect(url_for('report'))

@app.route('/appendix', methods=['GET', 'POST'])
def appendix():
    assessment_tables = session.get('assessment_tables', '')
    ...
    session['appendix_result'] = appendix_result
...
def report():
    ...
    intro_result = intro_chain.run({
        "child_dob": session['child_dob'], "requester": session['requester'],
        "pronoun": session['pronoun'], "assesment_administered": session['assesment_administered'],
        "assesment_time": session['assesment_time'], "meet_teacher": session['meet_teacher'],
        "meet_parent": session['meet_parent'], "meet_therapy_team": session['meet_therapy_team'],
        "lessons_observed": session['lessons_observed'], "sensory_profile": session['sensory_profile'],
        "parent_assesment": session['parent_assesment']
    })
    intro_content = format_report_content(intro_result)
    # print('\n\Intro Section : ', intro_content)

    external_report_result = session.get('external_report_result', '')
    external_report_content = format_report_content(external_report_result)
    # print('\n\nBackground Section External : ', external_report_content)

    initial_referral_result = session.get('initial_referral_result', '')
    ...
    views_result = session.get('views_result', '')
    ...
    clinical_analysis_result = session.get('clinical_analysis_result', '')
    ...
    return render_template(
        ...
        assessment_data=assessment_tables,
        clinical_analysis_content=clinical_analysis_content,
        summary_of_strengths_content=summary_of_strengths_content,
        summary_of_needs_content=summary_of_needs_content,
